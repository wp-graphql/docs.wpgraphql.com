...
Let’s say we stored some meta called color on our docs Post Type and we wanted to expose this on or
...
et’s say we wanted to register the same color field to ALL Post Types that are exposed in the Schema.
